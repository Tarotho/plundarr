...
def validate_series_yaml(data):
    ...
    for series in data:
        ...
        if "subtitles_language" in series and not isinstance(series["subtitles_language"], str):
            logging.error(f"Error: 'subtitles_language' debe ser una cadena en la serie: {series}")
            is_valid = False

        if "audio_language" in series and not isinstance(series["audio_language"], str):
            logging.error(f"Error: 'audio_language' debe ser una cadena en la serie: {series}")
            is_valid = False

    if is_valid:
        logging.info("El archivo YAML está correctamente configurado.")
    else:
        logging.error("El archivo YAML tiene errores. Modifíquelo antes de continuar")
